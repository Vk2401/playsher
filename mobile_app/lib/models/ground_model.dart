...
class GroundImage {
  ...
  const GroundImage({required this.id, required this.image, this.isPrimary = false});
...
class GroundModel {
  ...
  factory GroundModel.fromJson(Map<String, dynamic> json) {
    ...
    return GroundModel(
      ...
      images:      imgs.map((e) => GroundImage.fromJson(e as Map<String, dynamic>)).toList(),
  ...
  List<String> get sportNames =>
      groundSports.map((gs) => gs.sport?.name ?? '').where((n) => n.isNotEmpty).toList();
