import React, { useState, useMemo, useRef } from 'react'
import {
  Box,
  Button,
  TextField,
  Avatar,
  IconButton,
  Tooltip,
  Typography,
  Stack,
  InputAdornment,
  Select,
  MenuItem,
  FormControl,
  InputLabel,
  FormHelperText,
} from '@mui/material'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'

import AddIcon from '@mui/icons-material/Add'
import DeleteIcon from '@mui/icons-material/Delete'
import EditIcon from '@mui/icons-material/Edit'
import SearchIcon from '@mui/icons-material/Search'
import UploadFileIcon from '@mui/icons-material/UploadFile'
import FilterListIcon from '@mui/icons-material/FilterList'

import PageHeader from '../../components/ui/PageHeader.jsx'
import DataTable from '../../components/ui/DataTable.jsx'
import DrawerForm from '../../components/ui/DrawerForm.jsx'
import ConfirmDialog from '../../components/ui/ConfirmDialog.jsx'
import StatusChip from '../../components/ui/StatusChip.jsx'
import { useNotify } from '../../hooks/useNotify.js'
import { amenitiesApi } from '../../api/amenities.js'

const AMENITY_TYPES = ['venue', 'sport']
const INITIAL_FORM = { name: '', type: 'venue', icon: null }
const INITIAL_EDIT_FORM = { name: '', type: 'venue', icon: null }

export default function Amenities() {
  const queryClient = useQueryClient()
  const notify = useNotify()
  const fileInputRef = useRef(null)
  const editFileInputRef = useRef(null)

  const [search, setSearch] = useState('')
  const [typeFilter, setTypeFilter] = useState('all')
  const [drawerOpen, setDrawerOpen] = useState(false)
  const [form, setForm] = useState(INITIAL_FORM)
  const [iconPreview, setIconPreview] = useState(null)
  const [formErrors, setFormErrors] = useState({})

  const [deleteTarget, setDeleteTarget] = useState(null) // { id, name }

  // ── Edit state ─────────────────────────────────────────────────────────────
  const [editTarget, setEditTarget] = useState(null) // { id, name, type }
  const [editForm, setEditForm] = useState(INITIAL_EDIT_FORM)
  const [editIconPreview, setEditIconPreview] = useState(null)
  const [editFormErrors, setEditFormErrors] = useState({})

  // ── Query ──────────────────────────────────────────────────────────────────
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin', 'amenities'],
    queryFn: () => amenitiesApi.getAll(),
    select: (res) => res.data?.amenities || res.data?.data || res.data || [],
  })

  const rows = useMemo(() => {
    const list = Array.isArray(data) ? data : []
    return list.filter((r) => {
      const matchType = typeFilter === 'all' || r.type === typeFilter
      const matchSearch = !search.trim() || r.name?.toLowerCase().includes(search.toLowerCase())
      return matchType && matchSearch
    })
  }, [data, search, typeFilter])

  // ── Mutations ──────────────────────────────────────────────────────────────
  const createMutation = useMutation({
    mutationFn: (fd) => amenitiesApi.create(fd),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'amenities'] })
      notify.success('Amenity created successfully')
      handleCloseDrawer()
    },
    onError: (err) => {
      notify.error(err?.response?.data?.message || 'Failed to create amenity')
    },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, fd }) => amenitiesApi.update(id, fd),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'amenities'] })
      notify.success('Amenity updated successfully')
      handleCloseEditDrawer()
    },
    onError: (err) => {
      notify.error(err?.response?.data?.message || 'Failed to update amenity')
    },
  })

  const deleteMutation = useMutation({
    mutationFn: (id) => amenitiesApi.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'amenities'] })
      notify.success(`"${deleteTarget?.name}" deleted`)
      setDeleteTarget(null)
    },
    onError: (err) => {
      notify.error(err?.response?.data?.message || 'Failed to delete amenity')
    },
  })

  // ── Create Drawer helpers ──────────────────────────────────────────────────
  const handleOpenDrawer = () => {
    setForm(INITIAL_FORM)
    setIconPreview(null)
    setFormErrors({})
    setDrawerOpen(true)
  }

  const handleCloseDrawer = () => {
    setDrawerOpen(false)
    setForm(INITIAL_FORM)
    setIconPreview(null)
    setFormErrors({})
  }

  const handleFieldChange = (e) => {
    const { name, value } = e.target
    setForm((prev) => ({ ...prev, [name]: value }))
    if (formErrors[name]) setFormErrors((prev) => ({ ...prev, [name]: '' }))
  }

  const handleFileChange = (e) => {
    const file = e.target.files?.[0]
    if (!file) return
    setForm((prev) => ({ ...prev, icon: file }))
    setIconPreview(URL.createObjectURL(file))
    if (formErrors.icon) setFormErrors((prev) => ({ ...prev, icon: '' }))
  }

  const validate = () => {
    const errs = {}
    if (!form.name.trim()) errs.name = 'Name is required'
    if (!form.type) errs.type = 'Type is required'
    setFormErrors(errs)
    return Object.keys(errs).length === 0
  }

  const handleSubmit = () => {
    if (!validate()) return
    const fd = new FormData()
    fd.append('name', form.name.trim())
    fd.append('type', form.type)
    if (form.icon) fd.append('icon', form.icon)
    createMutation.mutate(fd)
  }

  // ── Edit Drawer helpers ────────────────────────────────────────────────────
  const handleOpenEditDrawer = (row) => {
    setEditTarget({ id: row.id, name: row.name, type: row.type })
    setEditForm({ name: row.name || '', type: row.type || 'venue', icon: null })
    setEditIconPreview(row.icon_url || row.icon || row.image_url || null)
    setEditFormErrors({})
  }

  const handleCloseEditDrawer = () => {
    setEditTarget(null)
    setEditForm(INITIAL_EDIT_FORM)
    setEditIconPreview(null)
    setEditFormErrors({})
  }

  const handleEditFieldChange = (e) => {
    const { name, value } = e.target
    setEditForm((prev) => ({ ...prev, [name]: value }))
    if (editFormErrors[name]) setEditFormErrors((prev) => ({ ...prev, [name]: '' }))
  }

  const handleEditFileChange = (e) => {
    const file = e.target.files?.[0]
    if (!file) return
    setEditForm((prev) => ({ ...prev, icon: file }))
    setEditIconPreview(URL.createObjectURL(file))
    if (editFormErrors.icon) setEditFormErrors((prev) => ({ ...prev, icon: '' }))
  }

  const validateEdit = () => {
    const errs = {}
    if (!editForm.name.trim()) errs.name = 'Name is required'
    if (!editForm.type) errs.type = 'Type is required'
    setEditFormErrors(errs)
    return Object.keys(errs).length === 0
  }

  const handleEditSubmit = () => {
    if (!validateEdit()) return
    const fd = new FormData()
    fd.append('name', editForm.name.trim())
    fd.append('type', editForm.type)
    if (editForm.icon) fd.append('icon', editForm.icon)
    updateMutation.mutate({ id: editTarget.id, fd })
  }

  // ── Columns ────────────────────────────────────────────────────────────────
  const columns = [
    {
      field: 'id',
      headerName: 'ID',
      width: 70,
      renderCell: ({ value }) => (
        <Typography variant="body2" color="text.secondary">
          #{value}
        </Typography>
      ),
    },
    {
      field: 'icon_url',
      headerName: 'Icon',
      width: 76,
      sortable: false,
      renderCell: ({ row }) => (
        <Avatar
          src={row.icon_url || row.icon || row.image_url}
          alt={row.name}
          variant="rounded"
          sx={{ width: 38, height: 38, bgcolor: 'secondary.light', fontSize: 16 }}
        >
          {row.name?.charAt(0).toUpperCase()}
        </Avatar>
      ),
    },
    {
      field: 'name',
      headerName: 'Name',
      flex: 1,
      minWidth: 150,
      renderCell: ({ value }) => (
        <Typography variant="body2" fontWeight={600}>
          {value}
        </Typography>
      ),
    },
    {
      field: 'type',
      headerName: 'Type',
      width: 110,
      renderCell: ({ value }) => (
        <StatusChip
          status={value === 'venue' ? 'active' : 'open'}
          sx={{ textTransform: 'capitalize' }}
        />
      ),
    },
    {
      field: 'status',
      headerName: 'Status',
      width: 100,
      renderCell: ({ value }) => <StatusChip status={value || 'active'} />,
    },
    {
      field: 'actions',
      headerName: 'Actions',
      width: 120,
      sortable: false,
      align: 'center',
      headerAlign: 'center',
      renderCell: ({ row }) => (
        <Stack direction="row" spacing={0.5} justifyContent="center">
          <Tooltip title="Edit Amenity">
            <IconButton
              size="small"
              color="primary"
              onClick={() => handleOpenEditDrawer(row)}
            >
              <EditIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Delete Amenity">
            <IconButton
              size="small"
              color="error"
              onClick={() => setDeleteTarget({ id: row.id, name: row.name })}
            >
              <DeleteIcon fontSize="small" />
            </IconButton>
          </Tooltip>
        </Stack>
      ),
    },
  ]

  return (
    <Box>
      <PageHeader
        title="Amenities"
        subtitle="Manage venue and sport amenities"
        breadcrumbs={[
          { label: 'Admin', href: '/admin' },
          { label: 'Amenities' },
        ]}
        actions={
          <Button
            variant="contained"
            startIcon={<AddIcon />}
            onClick={handleOpenDrawer}
          >
            Add Amenity
          </Button>
        }
      />

      {/* Filters row */}
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} mb={2} alignItems="flex-start">
        <TextField
          size="small"
          placeholder="Search by name…"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          sx={{ width: 300 }}
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
                <SearchIcon fontSize="small" />
              </InputAdornment>
            ),
          }}
        />

        <FormControl size="small" sx={{ minWidth: 160 }}>
          <InputLabel>
            <Box component="span" display="flex" alignItems="center" gap={0.5}>
              <FilterListIcon sx={{ fontSize: 16 }} />
              Type
            </Box>
          </InputLabel>
          <Select
            value={typeFilter}
            label="Type"
            onChange={(e) => setTypeFilter(e.target.value)}
          >
            <MenuItem value="all">All Types</MenuItem>
            {AMENITY_TYPES.map((t) => (
              <MenuItem key={t} value={t} sx={{ textTransform: 'capitalize' }}>
                {t.charAt(0).toUpperCase() + t.slice(1)}
              </MenuItem>
            ))}
          </Select>
        </FormControl>

        {rows.length !== (Array.isArray(data) ? data.length : 0) && (
          <Typography variant="body2" color="text.secondary" alignSelf="center">
            Showing {rows.length} of {Array.isArray(data) ? data.length : 0} amenities
          </Typography>
        )}
      </Stack>

      {/* Data table */}
      <DataTable
        rows={rows}
        columns={columns}
        loading={isLoading}
        error={error}
      />

      {/* ── Create Drawer ─────────────────────────────────────────────────── */}
      <DrawerForm
        open={drawerOpen}
        onClose={handleCloseDrawer}
        title="Add New Amenity"
        onSubmit={handleSubmit}
        loading={createMutation.isPending}
        submitLabel="Create Amenity"
      >
        <Stack spacing={2.5}>
          <TextField
            label="Amenity Name"
            name="name"
            value={form.name}
            onChange={handleFieldChange}
            error={!!formErrors.name}
            helperText={formErrors.name}
            fullWidth
            required
            autoFocus
          />

          <FormControl fullWidth required error={!!formErrors.type}>
            <InputLabel>Type</InputLabel>
            <Select
              name="type"
              value={form.type}
              label="Type"
              onChange={handleFieldChange}
            >
              {AMENITY_TYPES.map((t) => (
                <MenuItem key={t} value={t} sx={{ textTransform: 'capitalize' }}>
                  {t.charAt(0).toUpperCase() + t.slice(1)}
                </MenuItem>
              ))}
            </Select>
            {formErrors.type && <FormHelperText>{formErrors.type}</FormHelperText>}
          </FormControl>

          {/* Icon upload */}
          <Box>
            <Typography variant="caption" color="text.secondary" display="block" mb={1}>
              Icon / Image (optional)
            </Typography>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
              style={{ display: 'none' }}
              onChange={handleFileChange}
            />
            <Button
              variant="outlined"
              startIcon={<UploadFileIcon />}
              onClick={() => fileInputRef.current?.click()}
              size="small"
              sx={{ mb: 1 }}
            >
              {form.icon ? 'Change Icon' : 'Upload Icon'}
            </Button>
            {form.icon && (
              <Typography variant="caption" color="text.secondary" display="block">
                {form.icon.name}
              </Typography>
            )}
            {iconPreview && (
              <Box mt={1.5}>
                <Avatar
                  src={iconPreview}
                  variant="rounded"
                  sx={{ width: 72, height: 72, border: '1px solid', borderColor: 'divider' }}
                />
              </Box>
            )}
          </Box>
        </Stack>
      </DrawerForm>

      {/* ── Edit Drawer ───────────────────────────────────────────────────── */}
      <DrawerForm
        open={!!editTarget}
        onClose={handleCloseEditDrawer}
        title="Edit Amenity"
        onSubmit={handleEditSubmit}
        loading={updateMutation.isPending}
        submitLabel="Save Changes"
      >
        <Stack spacing={2.5}>
          <TextField
            label="Amenity Name"
            name="name"
            value={editForm.name}
            onChange={handleEditFieldChange}
            error={!!editFormErrors.name}
            helperText={editFormErrors.name}
            fullWidth
            required
            autoFocus
          />

          <FormControl fullWidth required error={!!editFormErrors.type}>
            <InputLabel>Type</InputLabel>
            <Select
              name="type"
              value={editForm.type}
              label="Type"
              onChange={handleEditFieldChange}
            >
              {AMENITY_TYPES.map((t) => (
                <MenuItem key={t} value={t} sx={{ textTransform: 'capitalize' }}>
                  {t.charAt(0).toUpperCase() + t.slice(1)}
                </MenuItem>
              ))}
            </Select>
            {editFormErrors.type && <FormHelperText>{editFormErrors.type}</FormHelperText>}
          </FormControl>

          {/* Icon upload */}
          <Box>
            <Typography variant="caption" color="text.secondary" display="block" mb={1}>
              Icon / Image (optional)
            </Typography>
            <input
              ref={editFileInputRef}
              type="file"
              accept="image/*"
              style={{ display: 'none' }}
              onChange={handleEditFileChange}
            />
            <Button
              variant="outlined"
              startIcon={<UploadFileIcon />}
              onClick={() => editFileInputRef.current?.click()}
              size="small"
              sx={{ mb: 1 }}
            >
              {editForm.icon ? 'Change Icon' : 'Upload Icon'}
            </Button>
            {editForm.icon && (
              <Typography variant="caption" color="text.secondary" display="block">
                {editForm.icon.name}
              </Typography>
            )}
            {editIconPreview && (
              <Box mt={1.5}>
                <Avatar
                  src={editIconPreview}
                  variant="rounded"
                  sx={{ width: 72, height: 72, border: '1px solid', borderColor: 'divider' }}
                />
              </Box>
            )}
          </Box>
        </Stack>
      </DrawerForm>

      {/* ── Delete Confirm ────────────────────────────────────────────────── */}
      <ConfirmDialog
        open={!!deleteTarget}
        onClose={() => setDeleteTarget(null)}
        onConfirm={() => deleteMutation.mutate(deleteTarget?.id)}
        loading={deleteMutation.isPending}
        title="Delete Amenity"
        message={`Permanently delete "${deleteTarget?.name}"? This action cannot be undone.`}
        confirmLabel="Delete"
        confirmColor="error"
      />
    </Box>
  )
}
